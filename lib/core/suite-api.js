 ...
 * });
   * ```
   * @param {String} suite_id 企业号的 suite_id
   * @param {String} suite_secret 在公众平台上申请得到的suite_secret
   * @param {Generator} getToken 可选的。获取全局token对象的方法，多进程模式部署时需在意
   * @param {Generator} saveToken 可选的。保存全局token对象的方法，多进程模式部署时需在意
   */
  constructor(suite_id, suite_secret, getToken, saveToken) {
    super({
      suite_id,
      suite_secret
    }, getToken, saveToken)
  ...
  async resolveAccessToken() {
    const {suite_id, suite_secret} = this.config
